			...
			<div className="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
				{props.products.map((product) => (
					<Link
						...
						className="group p-1 bg-white transition ease-in-out delay-150 hover:p-0 shadow-lg"
					>
						<div className="relative aspect-w-10 aspect-h-11 w-full overflow-hidden rounded-lg bg-gray-200">
							...
							<img
								...
								alt={product.title}
								className="h-full w-full object-cover object-center transition ease-in-out delay-150 group-hover:opacity-75"
							/>
						</div>
						...
						<div className="flex items-center">
							{product.discount > 0 ? (
								<>
									...
									<p className="my-2 text-lg font-medium text-red-new">$ {product.discountedPrice}</p>
								</>
							) : (
								<p className="my-2 mx-3 text-lg font-medium text-slate-900">$ {product.discountedPrice}</p>
							)}
						</div>
					</Link>
				))}
